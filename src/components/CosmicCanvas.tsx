...
function SceneInteraction({ mousePosRef }: { mousePosRef: React.MutableRefObject<THREE.Vector3 | null> }) {
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (e: PointerEvent) => {
      // Ignore click if it's on any interactive elements in the portfolio
      ...
      if (target && (
        ...
        target.closest('a') || 
        target.closest('form') ||
        target.closest('.interactive-portfolio')
      )) {
        return;
      }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Ignore key events if the user is typing in form inputs
      const target = e.target as HTMLElement;
